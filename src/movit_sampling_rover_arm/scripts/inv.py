#!/usr/bin/env python3
                                 # X Y Z_script  + tolerence msg
                                 #position_only_ik:true!              (Currently_Working_with!)
import rospy
...
def move_arm_to_sequential_xyz_flexible_orientation():
    ...
    end-effector will be determined by the Inverse Kinematics solver,s
    allowing flexible orinetation within a given tolerance.
    """
    ...
    move_group_arm.set_goal_tolerance(custom_joint_tolerance) #overights whats in the yaml file
    ...
    target_xyz_points = [
        (0, 0.82, 0.09),        #middle rock
        (-0.201195, 0.78, 0.09),     #left rock
        (0.2, 0.77, 0.09),      #Right rock
        (0.15,0.85,0.09),
        (0.1,0.75,0.09),
        (0.24,0.8,0.09),
        (-0.001682, 0.823266, 0.09), 
    ]
    rospy.loginfo("\nAttempting to prepick pose.")
    move_group_arm.set_named_target("pre_pick")
    ...
    for i, (x, y, z) in enumerate(target_xyz_points):
        ...
        # Deleting constrains before attempting to pose0
        ...
        #Getting the current endeffector ,,, Pose(XYZ,RPY)
        ...
        # Create orientation tolerance constraint
        oc = OrientationConstraint()
        oc.link_name = eef_link
        oc.header.frame_id = move_group_arm.get_planning_frame()
        oc.orientation = current_pose.orientation
        # ±30 degrees = 60° total range → 30° in each direction
        oc.absolute_x_axis_tolerance = math.radians(20)
        oc.absolute_y_axis_tolerance = math.radians(20)
        oc.absolute_z_axis_tolerance = math.radians(180)
        oc.weight = 1.0
        constraints = Constraints()
        constraints.orientation_constraints.append(oc)
        move_group_arm.set_path_constraints(constraints)
        ...
        # Provide target position as a list/tuple 
        move_group_arm.set_position_target([x, y, z]) 

        #debugging for the SOLVER
        # Make sure we only create the service proxy once
        if 'ik_service' not in globals():
            rospy.wait_for_service('/compute_ik')
            ik_service = rospy.ServiceProxy('/compute_ik', GetPositionIK)
            rospy.loginfo("service created.")
        # Prepare IK request
        ik_req = GetPositionIKRequest()
        ik_req.ik_request.group_name = arm_group_name
        ik_req.ik_request.pose_stamped.header.frame_id = move_group_arm.get_planning_frame()
        ...
        ik_req.ik_request.pose_stamped.pose.orientation = current_pose.orientation  # Keep current orientation
        ik_req.ik_request.timeout.secs = 1
        # Call IK solver
        rospy.loginfo("Calling IK solver for target...")
        ik_res = ik_service(ik_req)
        if ik_res.error_code.val == ik_res.error_code.SUCCESS:
            rospy.loginfo(" IK solver returned a valid solution.")
            rospy.loginfo("Joint solution: %s", [round(j, 4) for j in ik_res.solution.joint_state.position])
        else:
            rospy.logwarn(" IK solver failed for target %d — skipping planner.", i+1)
        ...
        if not success_plan or plan_trajectory is None or len(plan_trajectory.joint_trajectory.points) == 0:
            ...
            #trying to plan from different pose
            rospy.loginfo("\nAttempting to prepick pose.")
            move_group_arm.set_named_target("pre_pick")
            success_final_home = move_group_arm.go(wait=True)
            #Trying to go the same target again
            move_group_arm.set_position_target([x, y, z]) 
            move_group_arm.set_planning_time(5.0)
            rospy.loginfo("Re-Planning trajectory...")
            plan_result = move_group_arm.plan()
            success_plan = plan_result[0]
            plan_trajectory = plan_result[1]
            if not success_plan or plan_trajectory is None or len(plan_trajectory.joint_trajectory.points) == 0:
                rospy.logwarn("re-Planning failed or returned an empty trajectory for Target %d. Skipping.", i+1)
                move_group_arm.stop()
                move_group_arm.clear_pose_targets() # Clear any remaining targets
                continue
            else:
                rospy.loginfo("Re-Planning successful. Executing trajectory...")
        else:
